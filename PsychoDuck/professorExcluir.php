	...
	<div class="w3-main w3-container">
		...
		<section class='form-container'>
			...
			<div>
				...
				$sql = "SELECT ID,Matricula,CPF,Nome, Nome_Curso as Curso, DATE_FORMAT(DT_Nascimento,'%d/%m/%Y') as DT_Nascimento, Foto FROM Usuario as U INNER JOIN Professor as P ON (P.fk_Usuario_ID = U.ID) INNER JOIN CURSO AS C ON (P.fk_Curso_ID_CUrso = C.ID_Curso)
				...
				if ($result = mysqli_query($conn, $sql)) {
					if (mysqli_num_rows($result) == 1) {
						...
						<div class="w3-container w3-purple" style='width:500px'>
							<h2>Exclusão de Professor
							</h2>
						</div>
						<form class="w3-container" action="professorExcluir_exe.php" method="post"
							onsubmit="return check(this.form)">
							<input type="hidden" id="Id" name="Id" value="<?php echo $row['ID']; ?>">
							<p>
								<label class='w3-text-IE'>Matricula:
